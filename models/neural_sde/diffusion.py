...
# models/neural_sde/diffusion.py
import torch
import torch.nn as nn
...
class DiffusionNet(nn.Module):
    """
    Neural network to model the diffusion coefficient in an SDE
    
    This network maps the current state and time to a positive-valued
    diffusion coefficient, allowing for state-dependent noise modeling.
    """
    def __init__(self, latent_dim, hidden_dim):
        super(DiffusionNet, self).__init__()
        
        self.net = nn.Sequential(
            nn.Linear(latent_dim + 1, hidden_dim),  # +1 for time input
            Swish(),
            nn.Linear(hidden_dim, hidden_dim),
            Swish(),
            nn.Linear(hidden_dim, latent_dim),
            nn.Softplus()  # Ensure positive diffusion coefficients
        )
    ...
    def forward(self, t, z):
        ...
        Returns:
            diffusion: Diffusion coefficient [batch_size, latent_dim]
        """
        # Ensure t has the right shape
        if isinstance(t, float) or t.dim() == 0:
            t_tensor = torch.full((z.shape[0], 1), t, device=z.device)
...
def euler_maruyama(drift_fn, diffusion_fn, z0, ts, noise=None):
    """
    Euler-Maruyama scheme for SDE integration
    
    Args:
        drift_fn: Drift function f(t, z)
        diffusion_fn: Diffusion function g(t, z)
        z0: Initial state [batch_size, latent_dim]
        ts: Time points [num_steps]
        noise: Pre-generated noise (optional)
        
    Returns:
        z_path: State trajectory [num_steps, batch_size, latent_dim]
    """
    batch_size, latent_dim = z0.shape
    device = z0.device
    num_steps = len(ts)
    ...
    z_path = torch.zeros(num_steps, batch_size, latent_dim, device=device)
    z_path[0] = z0
    
    # Generate noise if not provided
    if noise is None:
        noise = torch.randn(num_steps - 1, batch_size, latent_dim, device=device)
    ...
    for i in range(num_steps - 1):
        ...
        z_next = z + f * dt + g * torch.sqrt(dt) * noise[i]
        z_path[i+1] = z_next
    
    return z_path

def milstein(drift_fn, diffusion_fn, diffusion_gradient_fn, z0, ts, noise=None):
    """
    ...
    Args:
        drift_fn: Drift function f(t, z)
        diffusion_fn: Diffusion function g(t, z)
        diffusion_gradient_fn: Gradient of diffusion wrt state ∂g/∂z
        z0: Initial state [batch_size, latent_dim]
        ts: Time points [num_steps]
        noise: Pre-generated noise (optional)
    ...
    for i in range(num_steps - 1):
        ...
        dg_dz = diffusion_gradient_fn(t, z)
        
        # Milstein correction term
        milstein_term = 0.5 * g * dg_dz * (dW**2 - dt)
        
        # Update state using Milstein scheme
        z_next = z + f * dt + g * dW + milstein_term
        z_path[i+1] = z_next
